...
//	List<Product> findActiveProducts2();
	...
	@Query("SELECT t FROM Bus t where t.status = :status")
	List<Bus> findActiveBus(@Param("status") Boolean status);
	
	@Query("SELECT t FROM Bus t where t.startPoint = :boardPoint AND t.endPoint = :dropPoint")
	List<Bus> findBusesByRoute(@Param("boardPoint") String boardPoint, @Param("dropPoint") String dropPoint);

package com.ticketbooking.cpcontroller;

import java.util.Date;
import java.util.List;
import java.util.UUID;

import javax.servlet.http.HttpServletRequest;

//import org.apache.log4j.Logger;
import org.jboss.logging.Logger;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.ui.ModelMap;
import org.springframework.web.bind.annotation.ModelAttribute;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.bind.annotation.RequestParam;
import org.springframework.web.bind.annotation.ResponseBody;
import org.springframework.web.servlet.ModelAndView;

import com.ticketbooking.domain.About;
import com.ticketbooking.service.AboutService;
import com.ticketbooking.util.FileUtil;

@Controller
@RequestMapping("/cp")
public class AboutController 
{   	
	protected static Logger logger = Logger.getLogger("controller");
	
	@Autowired
	private AboutService aboutService;	
	
	@RequestMapping("/all/about")
	@ResponseBody
	public List<About> getAllProducts() 
	{
		return aboutService.findAll();
	}
	
	@RequestMapping(value = "/about", method = RequestMethod.GET)
	public String findAll(@RequestParam(name="success", required=false) String success, ModelMap model) {
		
		model.addAttribute("title", "About Management");
		model.addAttribute("userClickAboutManagement",true);
		
		About nEntity = new About();
		
		// assuming that the user is ADMIN
		// later we will fixed it based on user is SUPPLIER or ADMIN
		nEntity.setImage("ABT" + UUID.randomUUID().toString().substring(26).toUpperCase());
		nEntity.setStatus(true);
		nEntity.setCreatedBy("admin");
		nEntity.setCreatedDate(new Date().toString());
		nEntity.setModifiedBy("admin");
		nEntity.setModifiedDate(new Date().toString());
		
		model.addAttribute("about",nEntity);
			
		if(success != null && success.equals("about"))
		{
			model.addAttribute("message", "About Submitted Successfully");
		}
		
		return "cp/page";
	}
	
	
	@RequestMapping(value= "/about/add", method = RequestMethod.POST)
	public String add(@ModelAttribute("about") About entity,
			HttpServletRequest request)
	{
//		if(category.getId() == 0L)
//		{
		
		
			this.aboutService.create(entity);
//		}
		/*else
		{
			this.categoryService.update(category);
		}	*/
		
			logger.info("Added Successfully");
			
		if (!entity.getFile().getOriginalFilename().equals("")) {
			FileUtil.uploadFile(request, entity.getFile(), entity.getImage());
			
			logger.info("File Added Successfully" + entity.getFile().getOriginalFilename());
		}
			
		return "redirect:/cp/about?success=about";
	}
	
	@RequestMapping(value ="/about/delete/{id}")
    public String delete(@PathVariable("id") Long id)
	{	
        this.aboutService.delete(id);
        return "redirect:/cp/about";
    }
	
	@RequestMapping("/about/{id}")
	public String edit(@PathVariable Long id, Model model) 
	{
		model.addAttribute("title", "About Management");
		model.addAttribute("userClickAboutManagement", true);

		// Product nProduct = new Product();
		model.addAttribute("about", this.aboutService.findOne(id));

		return "cp/page";

	}
	
	@RequestMapping(value = "/about/{id}/activation", method=RequestMethod.GET)
	@ResponseBody
	public String managePostAbouttActivation(@PathVariable Long id) {		
		About entity = aboutService.findOne(id);
		Boolean isActive = entity.getStatus();
		entity.setStatus(!isActive);
		aboutService.create(entity);		
		return (isActive)? "About Dectivated Successfully!": "About ID Activated Successfully";
	}
	
	@RequestMapping(value = "/show/{id}/about")
    public String findOne(@PathVariable Long id, Model model) throws Exception
	{
		About entity = this.aboutService.findOne(id);

		if (entity == null) throw new Exception();

		// update the view count
		entity.setViews(entity.getViews() + 1);
		aboutService.update(entity);
		// ---------------------------

		model.addAttribute("title", entity.getName());
		model.addAttribute("about", entity);

		//mv.addObject("userClickShowProduct", true);
		
		return "about";
	}
	
}